...
public class WolfHeuristics extends Heuristics {
	...
	public int evaluateState() {
		...
		result = WEIGHT_WHITE_PAWNS * (NUM_WHITE - this.currentNumberOfWhite)
				+ WEIGHT_BLACK_PAWNS * this.currentNumberOfBlack + WEIGHT_OPEN_WAYS * this.NumberOfKingFreeWays()
				+ WEIGHT_BLACK_NEAR_KING * this.NumberOfBlackNearKing() + WEIGHT_THREAT + this.numberOfPawnsInDanger()
				+ WEIGHT_RHOMBUS + this.numberOfPawnsInRhombus();
		return result;
	}

	/**
	 * This method return the number of BlackPawn in good position GoodPosition = a
	 * pawn that is not in danger and can eat a white Pawn in the next move
	 * 
		...
		for (int i = 0; i < board[0].length; i++) {
			for (int j = 0; j < board[0].length; j++) {
				if (board[i][j].equals(State.Pawn.BLACK)) { // MATCH!
					...
					// ----------------------------------------------------------------------------------------//
					// check north
					if (row - 1 - 1 >= 0 && (board[row - 1 - 1][column - 1].equalsPawn(State.Pawn.WHITE.toString())
							|| board[row - 1 - 1][column - 1].equalsPawn(State.Pawn.KING.toString())
							...
							|| board[row - 1 - 1][column - 1].equalsPawn(State.Pawn.THRONE.toString()))) {// we have a
																											...
																											// cells at
																											// north
						if (row + 1 <= 9 && this.canPawnGoThere(row + 1, column))
							number++;
					}
					// check south
					if (row - 1 + 1 <= 8 && (board[row - 1 + 1][column - 1].equalsPawn(State.Pawn.WHITE.toString())
							|| board[row - 1 + 1][column - 1].equalsPawn(State.Pawn.KING.toString())
							|| this.isPositionCitadel(row - 1 + 1, column - 1)
							|| board[row - 1 + 1][column - 1].equalsPawn(State.Pawn.THRONE.toString()))) {// we have a
																											// potential
																											...
																											// cells at
																											// south
						if (row - 1 >= 1 && this.canPawnGoThere(row - 1, column))
							number++;
					}
					// check west
					if (column - 1 - 1 >= 0 && (board[row - 1][column - 1 - 1].equalsPawn(State.Pawn.WHITE.toString())
							|| board[row - 1][column - 1 - 1].equalsPawn(State.Pawn.KING.toString())
							...
							|| board[row - 1][column - 1 - 1].equalsPawn(State.Pawn.THRONE.toString()))) {// we have a
																											...
																											// cells at
																											// west
						if (column + 1 <= 9 && this.canPawnGoThere(row, column + 1))
							number++;
					}
					// check east
					if (column - 1 + 1 <= 8 && (board[row - 1][column - 1 + 1].equalsPawn(State.Pawn.WHITE.toString())
							|| board[row - 1][column - 1 + 1].equalsPawn(State.Pawn.KING.toString())
							|| this.isPositionCitadel(row - 1, column - 1 + 1)
							|| board[row - 1][column - 1 + 1].equalsPawn(State.Pawn.THRONE.toString()))) {// we have a
																											// potential
																											...
																											// cells at
																											// east
						if (column - 1 >= 1 && this.canPawnGoThere(row, column - 1))
							number++;
					}
				}
			}
	...
	protected boolean canPawnGoThere(int posRow, int posColumn) {
		...
		for (int i = posRow - 1 - 1; i >= 0; i--) {
			if (!board[i][posColumn - 1].equalsPawn(State.Pawn.EMPTY.toString())) {
				...
				if (board[i][posColumn - 1].equalsPawn(State.Pawn.WHITE.toString())
						|| board[i][posColumn - 1].equalsPawn(State.Pawn.KING.toString())) {// the first pawn founded is
																							// white! DANGER
					return true;
				} else
	 ...
	 * white/back pawns It is called at the beginning of every heuristic evaluation
	 * 
	 * @param none
	 * 
	 * @return void
	...
	private boolean isPawnNearThrone(int row, int column) {
		if (column == 4 && row == 5)
			return true; // pawn to the left of throne
		if (column == 6 && row == 5)
			return true; // pawn to the right of throne
		if (column == 5 && row == 6)
			return true; // pawn to the sud of throne
		if (column == 5 && row == 4)
			return true; // pawn to the north of throne
		return false;
	}

	private boolean isPawnNearCitadel(int row, int column) {
		// check north side
		if (row > 1 && this.isPositionCitadel(column, row - 1))
			return true;
		// check south side
		if (row < 9 && this.isPositionCitadel(column, row + 1))
			return true;
		// check east side
		if (column < 9 && this.isPositionCitadel(column + 1, row))
			return true;
		// check west side
		if (column > 1 && this.isPositionCitadel(column - 1, row))
			return true;
		return false;
	}
	...
	private int getBlackNumberToCaptureKing() {
		...
		if (kingPositionColumn == 5 && kingPositionRow == 5)
			return 4;
		// if king is adjacent throne
		if (isPawnNearThrone(kingPositionRow, kingPositionColumn))
			return 3;
		// if king is adjacent citadel
		if (isPawnNearCitadel(kingPositionRow, kingPositionColumn))
			return 1;
		return 2;
	}
	...
	private int NumberOfBlackNearKing() {
		...
		int number = 0;
		// check north
		if (kingPositionRow - 1 - 1 >= 0 && (state.getBoard()[kingPositionRow - 1 - 1][kingPositionColumn - 1]
				.equalsPawn(State.Pawn.BLACK.toString())
				|| this.isPositionCitadel(kingPositionRow - 1, kingPositionColumn))) {
			number++;
		}
		// check south
		if (kingPositionRow - 1 + 1 <= 8 && (state.getBoard()[kingPositionRow - 1 + 1][kingPositionColumn - 1]
				.equalsPawn(State.Pawn.BLACK.toString())
				|| this.isPositionCitadel(kingPositionRow + 1, kingPositionColumn))) {
			number++;
		}
		// check east
		if (kingPositionColumn - 1 + 1 <= 8 && (state.getBoard()[kingPositionRow - 1][kingPositionColumn - 1 + 1]
				.equalsPawn(State.Pawn.BLACK.toString())
				|| this.isPositionCitadel(kingPositionRow, kingPositionColumn + 1))) {
			number++;
		}
		// check west
		if (kingPositionColumn - 1 - 1 >= 0 && (state.getBoard()[kingPositionRow - 1][kingPositionColumn - 1 - 1]
				.equalsPawn(State.Pawn.BLACK.toString())
				|| this.isPositionCitadel(kingPositionRow, kingPositionColumn - 1))) {
			number++;
		}
	...
	/**
	 * this method check if a pawn of the the given colour can go in a certain given
	 * position
	 * 
	...
	private boolean canPawnGoThere(Position pos) {
		...
		for (int i = pos.getRow() - 1 - 1; i >= 0; i--) {
			if (!board[i][pos.getColumn() - 1].equalsPawn(State.Pawn.EMPTY.toString())) {
				// inside this if only if the cells is not empty and is not a citadel
				if (board[i][pos.getColumn() - 1].equalsPawn(State.Pawn.WHITE.toString())
						|| board[i][pos.getColumn() - 1].equalsPawn(State.Pawn.KING.toString())) {// the first pawn
																									// founded is white!
																									// DANGER
					return true;
				} else
	...
	private boolean isThisPawInDanger(int row, int column) {
		...
		if (row - 1 - 1 >= 0 && (board[row - 1 - 1][column - 1].equalsPawn(State.Pawn.WHITE.toString())
				|| board[row - 1 - 1][column - 1].equalsPawn(State.Pawn.KING.toString())
				|| this.isPositionCitadel(row - 1 - 1, column - 1)
				|| board[row - 1 - 1][column - 1].equalsPawn(State.Pawn.THRONE.toString()))) {// we have a potential
																								// threat on south side
																								// because of bad cells
																								// at north
			if (row + 1 <= 9 && this.canPawnGoThere(row + 1, column))
				return true;
		}
		...
		if (column - 1 - 1 >= 0 && (board[row - 1][column - 1 - 1].equalsPawn(State.Pawn.WHITE.toString())
				|| board[row - 1][column - 1 - 1].equalsPawn(State.Pawn.KING.toString())
				|| this.isPositionCitadel(row - 1, column - 1 - 1)
				|| board[row - 1][column - 1 - 1].equalsPawn(State.Pawn.THRONE.toString()))) {// we have a potential
																								// threat on east side
																								// because of bad cells
																								// at west
			if (column + 1 <= 9 && this.canPawnGoThere(row, column + 1))
	...
	/**
	 * This method calculate the number of black pawn that are already in rhombus
	 * cells *
	 * 
	...
	private int numberOfPawnsInRhombus() {
		...
		if (this.currentNumberOfBlack < 10 || this.currentNumberOfWhite < 4)
			return 0;
		int number = 0;

		int Rposition[][] = { { 1, 2 }, { 1, 6 }, { 2, 1 }, { 2, 7 },
